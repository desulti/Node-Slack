...
router.get("/slack/oauth_redirect", async (ctx) => {
  // Obtain user information from the OAuth response
  const user = token.authed_user;
  const userId = user.id;

  // Call the channels.join method to add the user to a specific channel
  const joinChannelResponse = await client.channels.join({
    channel: 'a-project', // Replace with the actual channel ID
    user: userId,
  });

  console.log(`channels.join response: ${JSON.stringify(joinChannelResponse, null, 2)}`);

  // Redirect the user to the next page or perform other actions
  ctx.redirect("/next_page");
